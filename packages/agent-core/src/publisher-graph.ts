...
export function createPublisherGraph(dependencies: PublisherGraphDependencies) {
  return new StateGraph(publisherState)
    .addNode('check_advertiser', async (state) => ({
      policyAllowed: await dependencies.checkAdvertiser(state.offerId, state.advertiserId),
    }))
    ...
    .addConditionalEdges('check_advertiser', (state) => (state.policyAllowed ? 'evaluate_offer' : END))
